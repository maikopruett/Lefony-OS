...
#include <string.h>
namespace PrimeG2 { namespace LegacyAppStorage {
namespace {
...
bool Volume::protect(bool blocks[BlockCount]) {
  for(unsigned slot=0;slot<Slots;slot++) if(m_entries[slot].packageBytes) {
    if(!readHeader(slot,m_entries[slot].bank,m_header)) return false;
    for(unsigned i=0;i<get(m_header+64);i++) blocks[map(m_header,i)-FirstBlock]=true;
  }
  return true;
}
