...
bool AboutController::handleEvent(Ion::Events::Event event) {
  ...
  if (childLabel == I18n::Message::LefonyUsbStatus &&
      (event == Ion::Events::OK || event == Ion::Events::EXE || event == Ion::Events::Right)) {
    Container::activeApp()->displayModalViewController(&m_usbController,
      0.f, 0.f, 0, 0, 0, 0);
    return true;
  }
  ...
  if (event == Ion::Events::OK || event == Ion::Events::EXE || event == Ion::Events::Right) {
    ...
    if (!(event == Ion::Events::Right)) {
      ...
      if (childLabel == I18n::Message::UpsilonVersion) {
        // This is the Lefony release identity, not the upstream Upsilon version.
        return true;
      }
      ...
      if(childLabel == I18n::Message::Battery){
        ...
        char batteryLevel[5];
        if(strchr(myCell->accessoryText(), '%') == NULL) {
          if (!PrimeG2::Services::batteryEstimateIsCalibrated() &&
              !PrimeG2::Services::batteryFull()) {
            myCell->setAccessoryText("--%");
            return true;
          }
          int batteryLen = Poincare::Integer(PrimeG2::Services::batteryPercent()).serialize(batteryLevel, 4);
...
void AboutController::willDisplayCellForIndex(HighlightCell * cell, int index) {
  ...
  if (m_messageTreeModel->childAtIndex(i)->label() == I18n::Message::Contributors) {
    ...
    myCell->setAccessoryText(memUseBuffer);
  } else if (m_messageTreeModel->childAtIndex(i)->label() == I18n::Message::LefonyUsbStatus) {
    char status[] = "F:00000000 E:00";
    constexpr char hex[] = "0123456789ABCDEF";
    uint32_t flags = PrimeG2::USBDiagnostics::statusFlags();
    uint32_t error = PrimeG2::USBDiagnostics::firstErrorStep();
    for (int digit = 0; digit < 8; digit++) {
      status[2 + digit] = hex[(flags >> (28 - digit * 4)) & 15];
    }
    status[13] = hex[(error >> 4) & 15];
    status[14] = hex[error & 15];
    static_cast<MessageTableCellWithBuffer *>(cell)->setAccessoryText(status);
  } else {
    MessageTableCellWithBuffer * myCell = (MessageTableCellWithBuffer *)cell;
