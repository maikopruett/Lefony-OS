...
ARTIFACT_DIR=${NATIVE_STORAGE_FUZZ_TEST_DIR:-}
if [ -z "$ARTIFACT_DIR" ]; then
  ARTIFACT_DIR=$(mktemp -d "$REPO_DIR/build/prime-g2-native-storage-fuzz.XXXXXX")
fi
...
python3 "$REPO_DIR/vm/native-storage-fuzz.py" --socket "$INPUT_SOCKET"
