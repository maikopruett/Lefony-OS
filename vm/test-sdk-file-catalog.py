...
"""Installed ordinary C metadata APIs, pagination, errors, cold restart and fallback."""
import argparse
...
def main():
    ...
    parser.add_argument('--old-firmware',type=Path,required=True,help='Preserved API 5 or earlier ELF')
    args=parser.parse_args();output=ROOT/'build/sdk-file-catalog/arm';output.mkdir(parents=True,exist_ok=True)
    cases=[]
    with tempfile.TemporaryDirectory(prefix='lefony-catalog-') as temp:
        ...
        for mode in ('normal','denied','unsupported'):
            ...
            shutil.copyfile(ROOT/'tests/native/sdk_file_catalog.c',project/'src/main.c')
            write_json(project/'app.json',{'abi':1,'id':'file-catalog','name':'File Catalog',
                'version':'1.0.0','license':'GPL-3.0-or-later','schema':1,'data_schema':0,
                'minimum_api':6 if mode=='normal' else 3,
                'required_capabilities':152 if mode=='normal' else 24,'optional_capabilities':0})
            ...
            for cold in ([False,True] if mode=='normal' else [False]):
                    ...
                    try:
                        ...
                        while not int(channel.command('APP DIAG 15').split()[1]):
                            ...
                            if fault:
                                ...
                                raise AssertionError(f'{mode}/{cold}: {fault}\n{location}')
                ...
                with opened(project,'catalog') as (workspace,_):
                    ...
                    if mode=='normal':
                        exported=retained/('cold-value.bin' if cold else 'value.bin')
                        subprocess.run([reader,'export-file',workspace/'nand.overlay','file-catalog','folder/value',exported],
                            check=True,stdout=subprocess.DEVNULL,timeout=30)
                        assert exported.read_bytes()==b'nested record'
                assert result['result']==1 and result['os_responsive'],result
                cases.append({'mode':mode,'cold':cold,'runtime':result});print('PASS:',mode,'cold' if cold else 'first',flush=True)
    sources=['tests/native/sdk_file_catalog.c','vm/test-sdk-file-catalog.py','sdk/lib/newlib/files.c',
        'sdk/include/lefony/files_wire.h','sdk/include/lefony/files.h',
        *['ports/lefony-prime-g2/ion/src/prime_g2/'+p for p in
          ('app_storage.cpp','app_document_store.cpp','app_file_session.cpp','native_app.cpp')]]
