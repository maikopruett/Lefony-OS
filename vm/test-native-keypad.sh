...
TEST_DIR=${NATIVE_KEYPAD_TEST_DIR:-}
if [ -z "$TEST_DIR" ]; then
  TEST_DIR=$(mktemp -d "$REPO_DIR/build/prime-g2-native-keypad.XXXXXX")
fi
...
python3 "$REPO_DIR/vm/native-keypad-test.py" --socket "$SOCKET"
python3 "$REPO_DIR/vm/qmp-screendump.py" "$QMP_SOCKET" "$TEST_DIR/keymap.ppm"
test -s "$TEST_DIR/keymap.ppm"
